...
function getWordedDescription(race, gender, job) {
    const description = 'Wearing a dark brown leather apron and light blue cloathes';
    return description
}
...
async function clickButton(querySelection) {
    try { await page.waitForSelector(querySelection) } catch (e) { console.log(`trying to get query selection ${querySelection} `); throw e }
    try { await page.click(querySelection) } catch (e) { throw e }
}

async function getDescription(npc) {
    //if we havn't opened a browser - open a browser
    if (!page) {
        await pupLauncher()
    }
    npc.description = getWordedDescription(npc.race, npc.gender, npc.job)
    if (npc.gender === 'male') { clickButton(`body > div > div > app-root > app-race-selector > div:nth-child(3) > a:nth-child(2) > div`) }
    else { clickButton(`body > div > div > app-root > app-race-selector > div:nth-child(3) > a:nth-child(3) > div`) };
    clickButton(raceSelectors[npc.race] ? raceSelectors[npc.race] : raceSelectors.human)
    setTimeout(() => {
        page.screenshot({
            path: imagePath,
            clip: {
...
module.exports = {
    getDescription: async function (npc) {
        getDescription(npc)
    }
}
